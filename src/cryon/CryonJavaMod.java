...
import cryon.Injector.HeatBoostTurret;
import cryon.Injector.MeltingDrillInjector;
import cryon.TechTree.AravisTechTree;
import cryon.TechTree.CryonTechTree;
import cryon.TechTree.ModPlanetTechTree;
import cryon.Type.*;
...
public class CryonJavaMod extends Mod {
    ...
    public void loadContent() {
        ...
        stackRouter = new StackRouter("stack-router") {{
            ...
            public void init() {
                super.init();
                stackRouter.requirements(Category.distribution, with(
                        CryonItems.ferrum, 20,
                        Items.copper, 15
                ));
            }};
        ...
        windTurbine = new WindTurbine("wind-turbine") {{
            size = 2;
            health = 260;
            clearAreaSize = 9;        // 以自身为中心，9x9范围内不能有其他建筑/障碍
            powerProduction = 3.5f;   // 满功率发电量
            spinUpTime = 60f * 10f;   // 大约10秒爬升到满功率
            spinDownMultiplier = 3f;  // 一旦被挡住，回落速度是爬升速度的3倍（更快停下来）
