...
// FluxBarrier
// The code here is based on ForceProjector.
public class FluxBarrier extends Block{
    ...
    public float heatDissipationRate = 200f;//每秒散发的热量

    public float heatCooldownImmuneTime = 60f; // 免疫
    ...
    protected static FluxBarrier paramBlock;
    ...
    protected static final Cons<Bullet> shieldConsumer = bullet -> {
        if(bullet.team != paramEntity.team && bullet.type.absorbable && !bullet.absorbed &&
            ...
            paramEntity.hit = 1f;
            float damage = bullet.type.shieldDamage(bullet);
            float heatGenerated = damage * damage * damage * 0.001f;

            paramEntity.shieldHeat += heatGenerated;
            if (paramEntity.heatCooldownImmune <= 0&&(paramEntity.heatCooldownTimer<=0||paramEntity.MaxheatCooldownTime<damage)){
                paramEntity.heatCooldownTimer = damage * 1.5f;

                paramEntity.MaxheatCooldownTime = damage;
            }
            if(paramEntity.shieldHeat > paramEntity.maxShieldHeat) {
                paramEntity.shieldHeat = paramEntity.maxShieldHeat;
            }
        }
    };
    protected static final Cons<Unit> unitConsumer = unit -> {
        float overlapDst = (unit.hitSize / 2f + paramEntity.realRadius()) - unit.dst(paramEntity);
        ...
        if (overlapDst > 0) {
            if (overlapDst > unit.hitSize * 1.5f) {
                ...
                unit.vel.setZero();
                unit.move(Tmp.v1.set(unit).sub(paramEntity).setLength(overlapDst + 0.01f));

                if (Mathf.chanceDelta(0.12f * Time.delta)) {
                    Fx.circleColorSpark.at(unit.x, unit.y, paramEntity.team.color);
                }
            }
        }
    };
    ...
    public void setBars() {
        ...
        addBar("heat", (ForceBuild entity) ->
                new Bar(
                        Stat.temperature.localized(),
                        Pal.lightishOrange,
                        () -> entity.shieldHeat / entity.maxShieldHeat  // ← 改这里
                ).blink(Color.white)
        );
    }
    ...
    public class ForceBuild extends Building implements Ranged, ExplosionShield{
        ...
        public void updateTile(){
            ...
            }else if(heatCooldownTimer <= 0){
            if(shieldHeat > 0 ){
                float dissipation = heatDissipationRate * Time.delta / 60f;
                shieldHeat = Math.max(0, shieldHeat - dissipation);
            }

            if(coolantConsumer != null && coolantConsumer.efficiency(this) > 0){
                coolantConsumer.update(this);
                float liquidHeat = liquids.current().heatCapacity;
                shieldHeat -= delta() * heatDissipationRate * liquidHeat * 0.5f;
                shieldHeat = Math.max(0, shieldHeat);
            }
            }
        ...
        public boolean absorbExplosion(float ex, float ey, float damage){
            boolean absorb = Intersector.isInRegularPolygon(sides, x, y, realRadius(), shieldRotation, ex, ey);
            if(absorb){
                absorbEffect.at(ex, ey);
                hit = 1f;
                shieldHeat += damage * crashDamageMultiplier;
                shieldHeat = Math.min(shieldHeat, maxShieldHeat);
            }
            return absorb;
        }
